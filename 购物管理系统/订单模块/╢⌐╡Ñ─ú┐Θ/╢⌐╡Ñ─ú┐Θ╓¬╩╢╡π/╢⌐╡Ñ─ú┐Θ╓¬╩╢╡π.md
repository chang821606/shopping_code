...
def get_address(self,request):
        ...
        """
        # get_user_by_request()方法为utils中定义的取用户方法.
...
def get_sku_list(self, sku, cart_dict=None,s_count=None):
        """
        :param sku: SKU对象
        :param cart_dict:购物车中的SKU
        :param s_count: 购买数量(立即购买功能)
        ...
        return sku_sale_attr_names, sku_sale_attr_vals, s_count
```
...
```python
 def get_order_list(self,skus,cart_dict=None,s_count=None):
        """
        :param skus: SKU对象列表
        :param cart_dict: 购物车中的商品
        :patam s_count: 购买数量(立即购买功能)
...
- 实例演示
  ...
  ```python
  # 说明:代码执行在达达电商的/home/tarena/generalProjects/PublicProjects/dadashop/dashop/order/views.py中
  import os
  print(os.getcwd()) #/home/tarena/generalProjects/PublicProjects/dadashop/dashop
  ```

`代码段`

```python
def get_order_string(self,order_id,total_amount):
        """
        :param order_id:
        :param total_amount:
        :return: order_string
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            total_amount=int(total_amount + 10),
...
if status == 0:
            ...
            if len(addresses_list) < 1:
                # 如果收货地址为空则返回响应码50100,约定为添加收获地址
                return http.JsonResponse({'code': 50100, 'errmsg': '用户未添加收货地址'})
            # 2.组织商品数据
...
- transaction.atomic()
  ...
  创建一个事物保证在此事物内所有的数据库操作不会被提交,除非手动提交.

- transction.savepoint()

  返回值为**事物的保存点**.可以按需进行节点回滚.

- transaction.savepoint_rollback(**事物保存点**)

  回滚到某一事物点.

- transaction.savepoint_commit(**事物保存点**)
...
```python
	if status == 0:
            ...
            with transaction.atomic():  
                ...
                skus = SKU.objects.filter(id__in=cart_dict.keys())
                # sku_ids用以存储购物车中选中的商品,用作订单生成后删除redis中的商品.
                sku_ids = []
                for sku in skus:
                    selected = int(cart_dict[sku.id]['state']['selected'])
                    if selected == 0:
            ...
            # 4.生成支付宝支付链接
            order_string = self.get_order_string(order.order_id,order.total_amount)
            # 构建让用户跳转的支付链接
...
```python
 def get(self,request):
        ...
        if ali_verify is True:
            ...
            if order.status == 2:
                return redirect("http://"+IP_URL+":80/dadashop/templates/pay_success.html?status=1&order_id="+order_id+'&total_amount='+total_amount)
            # 主动查询
            else:
                ...
                if result.get("trade_status","") == "TRADE_SUCCESS":
                    ...
                    return redirect("http://"+IP_URL+":80/dadashop/templates/pay_success.html?status=2")
        else:
